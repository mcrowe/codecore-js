...
Example: Click handler on new shapes.
...
# Asynchronous vs. Synchronous calls and the event loop
...
# Prerequisite: Changine attributes dynamically.

## A) Adding / Removing classes

- 1) Create a class with styles you want to add.
...
- Make the rocket ship twice as big by adding a class to it in the console. Now remove the class.
- Make "item-1" have a red border by adding a class to it in the console. Now remove the class.
...
- Change the border color of all the list items to red, using the `css` method.
- Write a script so that when you click a list item, its border-width is set border-width is set to 10px.
...
```css
li {
  /* 1) Set original property value */
  opacity: 0;
  /* 2) Describe how to transition that property */
  transition: opacity 0.5s;
}

/* 3) Create a class you can add later to change the property */
li.active {
  opacity: 1;
}
...
## Exercise
- Make your own crazy 'drive' animation and execute it when "button-1" is clicked.
